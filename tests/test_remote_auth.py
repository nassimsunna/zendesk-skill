...
def test_oauth_issuer_with_trailing_slash_is_preserved_for_jwt_validation(monkeypatch):
    from zendesk_skill import remote_auth

    oauth_env(monkeypatch)
    monkeypatch.setenv("MCP_OAUTH_ISSUER", "https://auth.example.com/")
    monkeypatch.delenv("MCP_OAUTH_AUTHORIZATION_ENDPOINT", raising=False)
    monkeypatch.delenv("MCP_OAUTH_TOKEN_ENDPOINT", raising=False)
    ...
    def fake_decode(*args, **kwargs):
        seen["issuer"] = kwargs["issuer"]
        return {"sub": "user-1", "email": "nassim@samedaycustom.example", "scope": "zendesk:read", "iss": "https://auth.example.com/", "aud": "https://mcp.example.com/mcp", "exp": 9999999999}

    monkeypatch.setattr(remote_auth.jwt, "decode", fake_decode)

    valid, reason = remote_auth.validate_oauth_bearer("Bearer exact-trailing")
    metadata = remote_auth.authorization_server_metadata()
...
def test_oauth_issuer_without_trailing_slash_still_validates(monkeypatch):
    from zendesk_skill import remote_auth
    ...
    valid, reason = remote_auth.validate_oauth_bearer("Bearer exact-no-trailing")
...
def test_mismatched_oauth_issuer_remains_rejected(monkeypatch):
    ...
    monkeypatch.setenv("MCP_OAUTH_ISSUER", "https://auth.example.com/")
    monkeypatch.setattr(remote_auth.PyJWKClient, "get_signing_key_from_jwt", lambda self, token: SimpleNamespace(key="key"))
    monkeypatch.setattr(remote_auth.jwt, "decode", lambda *args, **kwargs: (_ for _ in ()).throw(InvalidIssuerError("bad iss")))

    valid, reason = remote_auth.validate_oauth_bearer("Bearer wrong-issuer")

...
- Remote users cannot choose file destinations. Remote tools do not expose `output_path`, `file_path`, or directory path inputs; any temporary response files are generated by the server inside `REMOTE_STORAGE_DIR` or a safe system temp folder.

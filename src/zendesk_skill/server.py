...
def create_remote_read_only_mcp() -> FastMCP:
    ...
    remote = FastMCP(
        "zendesk_skill_read_only",
        instructions=security_instructions(_START, _END),
        stateless_http=True,
        json_response=True,
    )
...
class _RemoteAuthMiddleware:
    """Raw ASGI OAuth protection that does not buffer Streamable HTTP traffic."""

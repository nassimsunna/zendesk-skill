...
def get_oauth_config() -> OAuthConfig:
    ...
    issuer = _strip_slash(os.environ.get("MCP_OAUTH_ISSUER", ""))
    jwks_url = os.environ.get("MCP_OAUTH_JWKS_URL") or (f"{issuer}/.well-known/jwks.json" if issuer else "")
    return OAuthConfig(
        issuer=issuer,
        audience=os.environ.get("MCP_OAUTH_AUDIENCE", ""),
        jwks_url=jwks_url,
        authorization_endpoint=os.environ.get("MCP_OAUTH_AUTHORIZATION_ENDPOINT", f"{issuer}/authorize" if issuer else ""),
        token_endpoint=os.environ.get("MCP_OAUTH_TOKEN_ENDPOINT", f"{issuer}/oauth/token" if issuer else ""),
